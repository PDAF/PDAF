!$Id: init_seek_pdaf.F90 1711 2016-12-20 10:17:58Z lnerger $
!BOP
!
! !ROUTINE: init_seek_pdaf --- Initialize state and modes for SEEK
!
! !INTERFACE:
SUBROUTINE init_seek_pdaf(filtertype, dim_p, rank, state_p, Uinv, &
     eofV_p, flag)
...
! ****************************
  ...
  mype0b: IF (mype_filter == 0) THEN
     ...
     DO domain = 2, npes_filter
        ...
        CALL MPI_send(state_p_temp, local_dims(domain), &
             MPI_REAL, domain - 1, 2, COMM_filter, MPIerr)
  ...
  ELSE mype0b
     ...
     CALL MPI_recv(eofV_p, dim_p * rank, MPI_REAL, &
          0, 1, COMM_filter, MPIstatus, MPIerr)
     CALL MPI_recv(state_p, dim_p, MPI_REAL, &
          0, 2, COMM_filter, MPIstatus, MPIerr)

  END IF mype0b

  ! *** broadcast eigenvalue matrix U to all filter PEs
  CALL MPI_bcast(Uinv, rank * rank, MPI_REAL, &
