!$Id: prepoststep_seek_offline.F90 1590 2015-06-12 11:59:35Z lnerger $
!BOP
!
! !ROUTINE: prepoststep_seek_offline --- Used-defined Pre/Poststep routine for PDAF
!
! !INTERFACE:
SUBROUTINE prepoststep_seek_offline(step, dim_p, dim_eof, dim_eof_p, dim_obs_p, &
...
! and after the re-diagonalization.
...
! in which the writing of the analysis state
! modes eofV and eigenvalue matrix Uinv
! can be performed.
!
...
! file.  We don't do an explicit writing of the 
! full ensemble here, as one would to in a real 
! offline implementation. For this, the user has 
! to implement writing routines that generate files,
! which are compatible with the model that has to 
! evolve the ensemble state. 
!
! !REVISION HISTORY:
...
! **********************
  ...
  IF (mype_filter == 0) THEN
     IF (firsttime) THEN
        WRITE (*, '(8x, a)') 'Analize forecasted state and error estimate'
     ELSE
        WRITE (*, '(8x, a)') 'Analize and write assimilated state and error estimate'
     END IF
  END IF
...
! *********************************
  ...
  CALL dgesv(dim_eof, dim_p, temp_Uinv, dim_eof, ipiv, &
...
! *******************
  ...
  IF (.not. firsttime) THEN
     ! ********************************************************
     ! *** Here is the place at which the analysis ensemble ***
     ! *** can be written into files for the forecast.      ***
     ! ********************************************************
  END IF

  ! Output RMS errors given by sampled and true covar matrices
  IF (mype_filter == 0) THEN
     IF (firstio) THEN
...
END SUBROUTINE prepoststep_seek_offline
