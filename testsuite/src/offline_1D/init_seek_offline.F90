!$Id: init_seek_offline.F90 1253 2012-01-30 18:56:08Z lnerger $
!BOP
!
! !ROUTINE: init_seek_offline --- Initialize state and modes for SEEK in offline mode
!
! !INTERFACE:
SUBROUTINE init_seek_offline(filtertype, dim_p, rank, state_p, Uinv, &
     eofV_p, flag)
...
! PE-local domain.
! For the offline mode, the modes and the state
! estimate will be typically read-in from files.
!
! The routine is called by all filter processes.
!
! This version is for the dummy model with domain 
! decomposition and PDAF in offline mode. For this
! example, we do not explicitly read the state 
! estimate and modes from files, but initialize 
! them as we do for the online mode. This example 
! first initializes the full ensemble on the 
! process with rank 0. Subsequently, the 
! sub-states are distributed according to the 
...
! **********************
  ...
  ! *** we still generate the state estimate and mode matrix   ***
  ! *** inside the code from a covariance matrix. In the real  ***
  ! *** application we would read this information from files! ***
  ! **************************************************************
...
END SUBROUTINE init_seek_offline
