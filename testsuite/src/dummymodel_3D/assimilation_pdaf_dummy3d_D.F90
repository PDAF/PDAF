!$Id: assimilation_pdaf_dummy3d_D.F90 783 2009-12-07 10:28:43Z lnerger $
!BOP
!
...
! ! Subroutine used in SEEK
  EXTERNAL :: prepoststep_seek    ! User supplied pre/poststep routine for SEEK
! ! Subroutines used in EnKF
  EXTERNAL :: prepoststep_enkf, & ! User supplied pre/poststep routine for EnKF
       add_obs_error, &           ! Add obs. error covariance R to HPH in EnKF
...
! *************************
  ...
  pdaf_modelloop: DO  
     ...
     IF (filtertype == 0) THEN
        ! SEEK
        CALL PDAF_get_state(nsteps, time, doexit, next_observation, &
             distribute_state, prepoststep_seek, status)
     ...
     ELSE IF (filtertype == 2) THEN
        ! EnKF
        CALL PDAF_get_state(nsteps, time, doexit, next_observation, &
             distribute_state, prepoststep_enkf, status)
...
! *******************************
        ...
        ! *** PDAF: Distinct calls due to different name of analysis routine ***
        IF (filtertype == 0) THEN
           CALL PDAF_put_state_seek(collect_state, init_dim_obs, obs_op, &
                init_obs, prepoststep_seek, prodRinvA, status)
        ELSE IF (filtertype == 1) THEN
        ...
        ELSE IF (filtertype == 2) THEN
           CALL PDAF_put_state_enkf(collect_state, init_dim_obs, obs_op, &
                init_obs, prepoststep_enkf, add_obs_error, init_obscovar, &
                status)
        ELSE IF (filtertype == 3) THEN
           ! local SEIK
!            CALL PDAF_put_state_lseik( &
!                 collect_state, init_dim_obs_full, obs_op_full, &
!                 init_obs_full, init_obs_local, prepoststep_seik, &
!                 prodRinvA_local, init_n_domains, init_dim_local, init_dim_obs_local, &
!                 global2local_state, local2global_state, global2local_obs, &
!                 init_obsvar, init_obsvar_local, status)
        END IF
