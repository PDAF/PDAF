!$Id: init_seek_dummy3d_D.F90 783 2009-12-07 10:28:43Z lnerger $
!BOP
!
...
! **********************
  ...
  mype0: IF (mype_filter == 0) THEN
     WRITE (*, '(/9x, a)') 'Generate initial EOF decomposed covariance matrix'
...
!       write (*,*) 'Domain: ',domain,' Offset: ',offset
        ...
        dim_state_l = dims_l_all(1, domain) * dims_l_all(2, domain) &
             * dims_l_all(3, domain)
        ALLOCATE(eofV_p_temp(dim_state_l, rank))
        ALLOCATE(state_p_temp(dim_state_l))
        IF (allocflag == 0) THEN
           ! count allocated memory
           CALL memcount(2, 'r', dim_state_l * rank + dim_state_l)
           allocflag = 1
        END IF
        ...
        ! Send sub-arrays
        CALL MPI_send(eofV_p_temp, rank * dim_state_l, &
             MPI_DOUBLE_PRECISION, domain - 1, 1, COMM_filter, MPIerr)
        CALL MPI_send(state_p_temp, dim_state_l, &
...
!     write (*,*) 'RECV: domain ',mype_filter+1,'dim_p',dim_p,'rank',rank
