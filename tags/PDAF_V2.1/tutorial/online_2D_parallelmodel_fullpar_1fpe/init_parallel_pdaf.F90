...
SUBROUTINE init_parallel_pdaf(dim_ens, screen)
  ...
  USE mod_parallel_pdaf, &        ! PDAF parallelization variables
       ...
       n_modeltasks, local_npes_model, task_id, COMM_couple
...
! *** local variables ***
  ...
  ! This step takes into account that a single separate process
  ! runs the analysis step in PDAF.
  ...
  ! Here we take into accout that a single separate process
  ! computes the analysis step - a single process of the
  ! model task is involved in coupling communication
