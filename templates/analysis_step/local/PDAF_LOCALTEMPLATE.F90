!> Module for LOCALTEMPLATE holding shared parameters and helper routines
!!
...
!! * 2025-02 - Lars Nerger - Initial template code based on LETKF
!! *  Other revisions - see repository log
!!
MODULE PDAF_LOCALTEMPLATE

  USE PDAF_mod_core, &            ! Variables for framework functionality
       ONLY: localfilter, covarloc, debug, dim_lag, member_save
...
! One can uses a different name for this variable
...
! +++ TEMPLATE: 
! +++ 'inloop' is essential since PDAF_reset_forget works differently if
! +++ it is called inside the local analysis loop from when it's called outside.
...
!>  PDAF-internal initialization of LOCALTEMPLATE
!!
...
! TEMPLATE: Adapt to include the name of the DA-method and perhaps a reference
    IF (verbose > 0) THEN
       WRITE(*, '(/a, 4x, a)') 'PDAF', '+++++++++++++++++++++++++++++++++++++++++++++++++++++++'
       WRITE(*, '(a, 4x, a)')  'PDAF', '+++                 LOCALTEMPLATE                   +++'
       WRITE(*, '(a, 4x, a)')  'PDAF', '+++                                                 +++'
       WRITE(*, '(a, 4x, a)')  'PDAF', '+++                                                 +++'
       WRITE(*, '(a, 4x, a)')  'PDAF', '+++                                                 +++'
       WRITE(*, '(a, 4x, a)')  'PDAF', '+++++++++++++++++++++++++++++++++++++++++++++++++++++++'
...
! TEMPLATE: These two parameters are declared in PDAFmod_core and we set them here to defaults
...
! TEMPLATE: This loop has to start with 3 because dim_p and dim_ens are set in PDAF before 
    DO i=3, dim_pint
       CALL PDAF_LOCALTEMPLATE_set_iparam(i, param_int(i), outflag)
    END DO
    DO i=1, dim_preal
       CALL PDAF_LOCALTEMPLATE_set_rparam(i, param_real(i), outflag)
    END DO
...
! TEMPLATE: Adapt if more subtypes exist
  ...
  END SUBROUTINE PDAF_LOCALTEMPLATE_init


!-------------------------------------------------------------------------------
!> Perform allocation of arrays for LOCALTEMPLATE.
!!
...
! *** Local variables
    INTEGER :: do_alloc_state_inc         ! Whether to alloc. state_inc_p
...
!-------------------------------------------------------------------------------
!>  Print information on configuration of LOCALTEMPLATE
!!
...
! TEMPLATE: Adapt output according to features and options of the DA-method
    ...
    writeout: IF (verbose > 0) THEN
       ...
       WRITE (*, '(/a, 4x, a)') 'PDAF', 'LOCALTEMPLATE configuration'
! TEMPLATE: Output on dim_ens should be generic
       ...
       IF (subtype == 0) THEN
             WRITE (*, '(a, 12x, a)') 'PDAF', '--> LOCALTEMPLATE analysis scheme'
...
! TEMPLATE: Any DA method should support type_obs_init to allow access to observations in prepoststep
  ...
  END SUBROUTINE PDAF_LOCALTEMPLATE_config


!-------------------------------------------------------------------------------
!> Set integer parameter specific for LOCALTEMPLATE
!!
  SUBROUTINE PDAF_LOCALTEMPLATE_set_iparam(id, value, flag)
...
! TEAMPLATE: This can be removed if there are no choices of different inflation methods
       ...
       END IF
    CASE(6)
! TEAMPLATE: This can be removed if there are no choices of different ensemble transformations
       type_trans = value
...
! to another id.
  ...
  END SUBROUTINE PDAF_LOCALTEMPLATE_set_iparam


!-------------------------------------------------------------------------------
!> Set real parameter specific for LOCALTEMPLATE
!!
  SUBROUTINE PDAF_LOCALTEMPLATE_set_rparam(id, value, flag)
...
! In any case one could also use a different name of the variable
  ...
  END SUBROUTINE PDAF_LOCALTEMPLATE_set_rparam

!-------------------------------------------------------------------------------
!> Information output on options for LOCALTEMPLATE
!!
!! Subroutine to perform information output on options
!! available for the LOCALTEMPLATE filter.
!!
  SUBROUTINE PDAF_LOCALTEMPLATE_options()
...
! TEMPLATE: Adapt output according to features and options of the DA-method
    ...
    WRITE(*, '(/a, 5x, a)') 'PDAF', 'Available options for LOCALTEMPLATE:'
...
!     WRITE(*, '(a, 12x, a)') 'PDAF', '>0: apply smoother up to specified lag'
    ...
    WRITE(*, '(a, 12x, a)') 'PDAF', '1: global adaptive forgetting factor'
    WRITE(*, '(a, 12x, a)') 'PDAF', '2: local adaptive forgetting factor'
! TEMPLATE: Adapt depending on whether ensemble transofrmation types are used
    ...
    WRITE(*, '(a, 7x, a)') 'PDAF', 'param_int(9): type_obs_init'
...
! TEMPLATE: Add further param_int options if they exist
    ...
    WRITE(*, '(a, 7x, a)') 'PDAF', 'param_real(1): forget'
...
! TEMPLATE: Add further param_real options if they exist
    ...
    WRITE(*, '(a, 11x, a)') 'PDAF', '>=1; not larger than total number of processors'
    ...
    WRITE(*, '(a, 5x, a)') &
         'PDAF', '+++++++++ End of option overview for the LOCALTEMPLATE ++++++++++'

  END SUBROUTINE PDAF_LOCALTEMPLATE_options
...
!! memory information for the LOCALTEMPLATE.
!!
...
! TEMPLATE: printtype=1 is generic and should not need changes
       ...
       IF (filterpe) THEN
          ...
          WRITE (*, '(a, 20x, a, F11.3, 1x, a)') 'PDAF', 'LOCALTEMPLATE analysis:', pdaf_time_tot(3), 's'
...
! TEMPLATE: This part likely need adaptions according to the filter
       IF (filterpe) THEN
          ! Filter-specific part
          WRITE (*, '(a, 10x, a, 17x, F11.3, 1x, a)') 'PDAF', 'LOCALTEMPLATE analysis:', pdaf_time_tot(3), 's'
...
! if one implements very detailed timers.
...
! TEMPLATE: For a domain-local method the outputs here are all generic since they relate to typical steps of a local filter
! For a more detailed timer example for printtype=5 see e.g. PDAF_etkf_memtime in PDAF_etkf.F90
       IF (filterpe) THEN
          ! Filter-specific part
          WRITE (*, '(a, 22x, a, F11.3, 1x, a)') 'PDAF', 'LOCALTEMPLATE analysis (3):', pdaf_time_tot(3), 's'  ! generic
          WRITE (*, '(a, 18x, a, F11.3, 1x, a)') 'PDAF', 'prepare observations (6):', pdaf_time_tot(6), 's'     ! generic
          WRITE (*, '(a, 19x, a, F11.3, 1x, a)') 'PDAF', 'compute ensemble mean (9):', pdaf_time_tot(9), 's'    ! generic
          WRITE (*, '(a, 19x, a, F11.3, 1x, a)') 'PDAF', 'global preparations (7):', pdaf_time_tot(7), 's'      ! generic
          WRITE (*, '(a, 19x, a, F11.3, 1x, a)') 'PDAF', 'local analysis loop (8):', pdaf_time_tot(8), 's'      ! generic
          WRITE (*, '(a, 24x, a, F11.3, 1x, a)') 'PDAF', 'global to local (10):', pdaf_time_tot(10), 's'       ! generic  
          WRITE (*, '(a, 18x, a, F11.3, 1x, a)') 'PDAF', 'localize observations (11):', pdaf_time_tot(11), 's'  ! generic
          WRITE (*, '(a, 25x, a, F11.3, 1x, a)') 'PDAF', 'local analysis (12):', pdaf_time_tot(12), 's'        ! generic
          WRITE (*, '(a, 24x, a, F11.3, 1x, a)') 'PDAF', 'local to global (14):', pdaf_time_tot(14), 's'       ! generic
          IF (dim_lag >0) &
