...
! **********************************
    ...
    IF (nx==36) THEN
       OPEN (12, file='../inputs_online.18x36/obs_step'//TRIM(stepstr)//'.txt', status='old')
    ELSEIF (nx==256) THEN
       OPEN (12, file='../inputs_online.128x256/obs_step'//TRIM(stepstr)//'.txt', status='old')
    ELSE
       OPEN (12, file='../inputs_online.512x2048obs_step'//TRIM(stepstr)//'.txt', status='old')
    END IF
    DO i = 1, ny
       READ (12, *) obs_field(i, :)
    END DO
