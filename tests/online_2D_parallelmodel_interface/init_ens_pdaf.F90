...
! *** INITIALIZATION ***
! **********************
write (*,*) 'dim, dim_ens', dim_p, dim_ens, flag
...
! ********************************
  ...
  DO member = 1, dim_ens
     WRITE (ensstr, '(i1)') member
        IF (nx==36) THEN
           OPEN(11, file = '../inputs_online.18x36/ens_'//TRIM(ensstr)//'.txt', status='old')
        ELSEIF (nx==256) THEN
           OPEN(11, file = '../inputs_online.128x256/ens_'//TRIM(ensstr)//'.txt', status='old')
        ELSE
           OPEN(11, file = '../inputs_online.512x2048/ens_'//TRIM(ensstr)//'.txt', status='old')
        END IF
