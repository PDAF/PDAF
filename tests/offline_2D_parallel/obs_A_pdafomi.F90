...
! *** Local variables ***
    ...
    INTEGER :: cnt_p, cnt0_p, cnt0       ! Counters
...
! **********************************
    ...
    IF (nx==36) THEN
       OPEN (12, file='../inputs_offline.18x36/obs.txt', status='old')
    ELSE IF (nx==256) THEN
       OPEN (12, file='../inputs_offline.256x128/obs.txt', status='old')
    ELSE IF (nx==512) THEN
       OPEN (12, file='../inputs_offline.512x512/obs.txt', status='old')
    ELSE IF (nx==2048) THEN
       OPEN (12, file='../inputs_offline.512x2048/obs.txt', status='old')
    END IF
    DO i = 1, ny
       READ (12, *) obs_field(i, :)
    END DO
...
! ***********************************************************
    ...
    DO j = 1, nx
       DO i= 1, ny
          cnt0 = cnt0 + 1
          IF (cnt0 > off_p .AND. cnt0 <= off_p+local_dims(mype_filter+1)) THEN
    ...
    haveobs: IF (dim_obs_p > 0) THEN
       ...
       DO j = 1, nx
          DO i= 1, ny
             cnt0 = cnt0 + 1
             IF (cnt0 >= off_p+1 .AND. cnt0 <= off_p+local_dims(mype_filter+1)) THEN
                cnt0_p = cnt0_p + 1
...
!! for each observation type.
!!
  SUBROUTINE localize_covar_serial_A(iobs, dim_p, dim_obs, HP_p, HXY_p, coords_p)
...
! *************************************
  ...
  END SUBROUTINE localize_covar_serial_A
