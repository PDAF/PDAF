...
"""
Plot functions for the Lorenz05b example of the PDAF test
...
def plot_eofs(filename, index):
    ...
    if index > 0:
        ax.set_title("Eigenvector {0:d} of Lorenz05b model".format(index))
    else:
        ax.set_title("Mean state of Lorenz05b model")


def plot_obs(filename, timestep):
    """
    Open a NetCDF file holding observations for the Lorenz05b model and
    ...
    ax.set_title(("Observations for Lorenz05b model at time {0:.3f} "
...
def plot_rms(filename, plot_forecast=False, plot_analysis=True):
    ...
    the Lorenz05b model and plos the true and estimated rms errors.
...
def plot_sigma(filename):
    ...
    the Lorenz05b model and plot the eigenvalue spectrum.
    ...
    ax.set_title("Eigenvalues of covariance matrix for Lorenz05b model")


def plot_state(filename, timestep, choice='t'):
    """
    Open NetCDF output from the Lorenz05b model and plot the state at
    the selected time.
    ...
    ax.set_title(("Lorenz05b model {0} at time {1:.3f} (time step {2:d})"
