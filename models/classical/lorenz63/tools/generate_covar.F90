...
! ****************************
  ...
  read_in: DO iter = step_start, step_start+(ntimes-1)*delta_step, delta_step
     ...
     startv(1) = 1
     countv(1) = dim_state
     stat(1) = NF90_GET_VAR(ncid_in, id_state, states(1 : dim_state, iter), start=startv, count=countv)
...
! *** Determine rank to write ***
  ...
  countv(1) = dim_state
  s = s + 1
  stat(s) = NF90_PUT_VAR(ncid_out, id_mean, meanstate, start=startv)
  ...
  writevectors: DO i = 1, rank
    ...
    ! Write singular vector
     ...
     countv(1) = dim_state
     s = 1
     stat(s) = NF90_PUT_VAR(ncid_out, id_svec, svdU(:,i), start=startv)
