...
!
! This example is for the dummy model in which all grid points
! are observed. For incomplete observations, the computation 
! of the distances has to be changed.
!
! !REVISION HISTORY:
! 2016-11 - Lars Nerger
...
! This example is for the case when all grid points are observed
...
! the distance, because half of HP and all of HPH live in observation space.
  ...
  DO j = 1, dim_obs
     DO i = 1, dim_p
        ! Compute weight
        distance = MIN(SQRT(REAL(j-i) * REAL(j-i)), ABS(SQRT(REAL(j-i) * REAL(j-i)) - dim_p))
