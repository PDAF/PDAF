!$Id: prepoststep_seek_dummy3d_D.F90 1549 2015-01-17 09:29:05Z lnerger $
!BOP
!
...
! analyzed, e.g. by computing the estimated 
...
! **********************
  ...
  IF (mype_filter == 0) THEN
     ...
     ELSE IF (step < 0) THEN
        WRITE (*, '(8x, a)') 'Analyze forecast state ensemble'
     END IF
  END IF
  ...
  ALLOCATE(eofVt(dim_eof, dim_var_p))
  ALLOCATE(ipiv(dim_eof))
  IF (allocflag == 0) THEN
     ! count allocated memory
     CALL memcount(3, 'r', 2 * dims(1) * dims(2) + dim_eof**2 &
          + (dim_eof + 1) * dim_var_p)
...
! *********************************
  ...
  DO j = 1, dim_eof
     DO i = 1, dim_var_p
        eofVt(j,i) = eofV_p(i,j)
     END DO
  END DO

  ! *** call solver - compute X = U V^T
  CALL dgesv(dim_eof, dim_var_p, temp_Uinv, dim_eof, ipiv, &
...
! ******************************
  ...
  IF (allocflag == 0) THEN
     ! count allocated memory
     CALL memcount(3, 'r', dim_p + 2 * dim_var_p + dims(1) * dims(2))
     allocflag = 1
  END IF

  ! Initialize temporary state vector
  state_p_tmp = state_p(1 : dim_p)
