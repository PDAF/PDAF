...
! *********************
    ...
    writeout: IF (verbose > 0) THEN
       ...
       IF (subtype == 0) THEN
          WRITE (*, '(a, 14x, a)') 'PDAF', '--> ENSRF with serial observation processing'
       END IF
       WRITE(*, '(a, 10x, a, i3)') &
            'PDAF', 'param_int(3) rank_ana_ensrf=', rank_ana_ensrf
!        IF (rank_ana_ensrf == 0) THEN
!           WRITE (*, '(a, 12x, a)') &
!                'PDAF', '---> analysis with direct inversion'
!        ELSE
!           WRITE (*, '(a, 12x, a, i5)') &
!                'PDAF', '--->analysis with pseudo-inverse of HPH, rank=', rank_ana_ensrf
!        END IF
       IF (dim_lag > 0) &
...
! ****************************
    ...
    CASE(3)
       IF (value==0 .AND. value < dim_ens) THEN
          rank_ana_ensrf = value
       ELSE
          WRITE (*,'(/5x, a/)') &
             'PDAF-ERROR(8): Invalid setting of param_int(3)/rank_ana_ensrf!'
          flag = 8
          rank_ana_ensrf = 0 ! Just for safety: Fall back to default
       END IF
    CASE(4)
       ! Not used
    CASE(5)
...
! *********************
    ...
    WRITE(*, '(a, 7x, a)') 'PDAF', '0: Standard ENSRF with serial observation processing'

    WRITE(*, '(a, 5x, a)') 'PDAF', '--- Integer parameters (Array param_int) ---'
    WRITE(*, '(a, 7x, a)') 'PDAF', 'param_int(1): Dimension of state vector (>0), required'
    WRITE(*, '(a, 7x, a)') 'PDAF', 'param_int(2): Ensemble size (>0), required'
    WRITE(*, '(a, 7x, a)') 'PDAF', 'param_int(3): rank_ana_ensrf'
...
! *******************************************************
       ...
       IF (filterpe) THEN
          ! Filter-specific part
          WRITE (*, '(a, 10x, a, 17x, F11.3, 1x, a)') 'PDAF', 'ENSRF analysis:', pdaf_time_tot(3), 's'
...
! *********************************************
       ...
       IF (filterpe) THEN
          ! Filter-specific part
          WRITE (*, '(a, 23x, a, F11.3, 1x, a)') 'PDAF', 'ENSRF analysis (3):', pdaf_time_tot(3), 's'
...
!! the analysis step of the domain-decomposed ENSRF. 
...
!! * Other revisions - see repository log
!!
  SUBROUTINE PDAF_ensrf_gather_resid(dim_obs, dim_obs_p, dim_ens, resid_p, resid)
...
! *** Clean up ***
  ...
  END SUBROUTINE PDAF_ensrf_gather_resid

!-------------------------------------------------------------------------------
!> Generate ensemble of observations for ENSRF
!!
!! This routine generates an ensemble of observations 
!! from a mean observation with prescribed error
!! covariance matrix for the ENSRF94/98.
...
!! Other revisions - see repository log
!!
  SUBROUTINE PDAF_ensrf_obs_ensemble(step, dim_obs_p, dim_obs, dim_ens, obsens_p, &
...
! *************************************
  ...
  ! *** to avoid a parallelization of the possible eigendecomposition.
    IF (debug>0) &
         WRITE (*,*) '++ PDAF-debug: ', debug, 'PDAF_ensrf_obs_ensemble -- call init_obs_covar'
    ...
    diagA: IF (.NOT. isdiag) THEN
       ...
       IF (debug>0) &
            WRITE (*,*) '++ PDAF-debug PDAF_ensrf_obs_ensemble:', debug, &
       ...
       IF (syev_info==0 .AND. debug>0) &
            WRITE (*,*) '++ PDAF-debug PDAF_ensrf_resample:', debug, &
...
! ****************
  ...
  END SUBROUTINE PDAF_ensrf_obs_ensemble
