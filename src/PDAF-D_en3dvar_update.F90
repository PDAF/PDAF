...
!BOP
!
! !ROUTINE: PDAF_en3dvar_update --- Control analysis update of the 3DVAR
!
! !INTERFACE:
SUBROUTINE  PDAF_en3dvar_update(step, dim_p, dim_obs_p, dim_ens, &
...
! *** Prestep for forecast ensemble ***
  ...
  IF (subtype > 0) THEN
     ! *** Now perform update of the ensemble perturbations using LETKF ***

     incremental_tmp = 2

     CALL  PDAF_letkf_update(step, dim_p, dim_obs_p, dim_ens, &
          state_p, Uinv, ens_p, state_inc_p, forget, &
          U_init_dim_obs_f, U_obs_op_f, U_init_obs_f, U_init_obs_l, U_prodRinvA_l, &
          U_init_n_domains_p, U_init_dim_l, U_init_dim_obs_l, U_g2l_state, U_l2g_state, &
          U_g2l_obs, U_init_obsvar, U_init_obsvar_l, U_prepoststep, screen, &
          0, incremental_tmp, type_forget, dim_lag, sens, cnt_maxlag, flag)

     ! *** Add state increment from 3D-Var to ensemble
...
END SUBROUTINE PDAF_en3dvar_update
