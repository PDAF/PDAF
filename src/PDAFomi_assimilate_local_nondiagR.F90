...
SUBROUTINE PDAFomi_assimilate_local_nondiagR(collect_state_pdaf, distribute_state_pdaf, &
          ...
          init_dim_l_pdaf, init_dim_obs_l_pdaf, prodRinvA_l_pdaf, g2l_state_pdaf, l2g_state_pdaf, &
...
! ! Names of external subroutines 
  ...
  EXTERNAL :: prodRinvA_l_pdaf         ! Provide product of inverse of R with matrix A
...
! **************************************************
  ...
  ELSE IF (TRIM(filterstr) == 'LNETF') THEN
     WRITE (*,*) 'PDAF-ERROR: Non-diagonal R matrix is not supported for LNETF'
     outflag=200
  ELSE IF (TRIM(filterstr) == 'LKNETF') THEN
     WRITE (*,*) 'PDAF-ERROR: Non-diagonal R matrix is not supported for LKNETF'
     outflag=200
  END IF
