...
!
!> Interfaces to PDAF for flexible parallelization mode for ensemble filters using PDAFlocal
!!
...
!!
!! The interface routines provided here are the older
!! PDAF-2 routines using the naming PDAFlocal_. 
!!
...
!!
MODULE PDAFlocal_put_state_ens

CONTAINS

!-------------------------------------------------------------------------------
!> Interface to transfer state to PDAF for LSEIK
!!
!! __Revision history:__
  ...
  completeforecast: IF (member == local_dim_ens + 1 &
     ...
     OnFilterPE: IF (filterpe) THEN
        CALL PDAFlseik_update(step_obs, dim_p, dim_obs, dim_ens, dim_ens-1, state, &
             Ainv, ens, U_init_dim_obs, U_obs_op, &
             U_init_obs, U_init_obs_l, U_prodRinvA_l, U_init_n_domains_p, U_init_dim_l, &
             U_init_dim_obs_l, PDAFlocal_g2l_cb, PDAFlocal_l2g_cb, U_g2l_obs, U_init_obsvar, &
...
!-------------------------------------------------------------------------------
!> Interface to transfer state to PDAF for LETKF
!!
!! __Revision history:__
  ...
  completeforecast: IF (member == local_dim_ens + 1 &
     ...
     OnFilterPE: IF (filterpe) THEN
        CALL PDAFletkf_update(step_obs, dim_p, dim_obs, dim_ens, state, &
             ...
             U_init_dim_obs_l, PDAFlocal_g2l_cb, PDAFlocal_l2g_cb, U_g2l_obs, &
...
!-------------------------------------------------------------------------------
!> Interface to transfer state to PDAF for LESTKF
!!
!! __Revision history:__
...
!-------------------------------------------------------------------------------
!> Interface to transfer state to PDAF for LNETF
!!
!! __Revision history:__
...
!-------------------------------------------------------------------------------
!> Interface to transfer state to PDAF for LKNETF
!!
!! __Revision history:__
...
END MODULE PDAFlocal_put_state_ens
