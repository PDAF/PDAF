...
!-------------------------------------------------------------------------------
!>  Print information on configuration of LNETF
!!
!!  !  This is a core routine of PDAF and   !
!!  !   should not be changed by the user   !
!!
!! __Revision history:__
!! * 2025-02 - Lars Nerger - Initial code by splitting from PDAF_lnetf_init
!! *  Other revisions - see repository log
!!
  SUBROUTINE PDAF_lnetf_config(subtype, verbose)
...
! *********************
  ...
  END SUBROUTINE PDAF_lnetf_config
...
! *********************
    ...
    WRITE(*, '(a, 11x, a)') 'PDAF', 'Note: This parameter has no influence on the LNETF assimilation result'
    WRITE(*, '(a, 12x, a)') 'PDAF', '0: Apply H to ensemble mean to compute innovation'
...
! *******************************************************
       ...
       IF (.not.offline_mode) THEN
          ...
          END IF
          WRITE (*, '(a, 12x, a, 4x, F11.3, 1x, a)') 'PDAF', 'MPI communication in PDAF:', pdaf_time_tot(4), 's'
...
!            WRITE (*, '(a, 14x, a, 8x, F11.3, 1x, a)') 'PDAF', 'PDAFomi_likelihood_l:', pdaf_time_tot(48), 's'
          ELSE
             ...
             END IF
             WRITE (*, '(a, 12x, a, 13x, F11.3, 1x, a)') 'PDAF', 'likelihood_l_pdaf:', pdaf_time_tot(48), 's'
             WRITE (*, '(a, 12x, a, 16x, F11.3, 1x, a)') 'PDAF', 'l2g_state_pdaf:', pdaf_time_tot(14), 's'
          END IF
...
! *********************************************
       ...
       IF (filterpe) THEN
          ...
          WRITE (*, '(a, 17x, a, F11.3, 1x, a)') 'PDAF', 'compute ensemble mean (9):', pdaf_time_tot(9), 's'
          WRITE (*, '(a, 19x, a, F11.3, 1x, a)') 'PDAF', 'global preparations (7):', pdaf_time_tot(7), 's'
          WRITE (*, '(a, 19x, a, F11.3, 1x, a)') 'PDAF', 'local analysis loop (8):', pdaf_time_tot(8), 's'
          WRITE (*, '(a, 24x, a, F11.3, 1x, a)') 'PDAF', 'global to local (10):', pdaf_time_tot(10), 's'
          WRITE (*, '(a, 18x, a, F11.3, 1x, a)') 'PDAF', 'localize observations (11):', pdaf_time_tot(11), 's'
          WRITE (*, '(a, 25x, a, F11.3, 1x, a)') 'PDAF', 'local analysis (12):', pdaf_time_tot(12), 's'
...
! *****************************************
       ...
       IF (filterpe) THEN
          ...
          WRITE (*, '(a, 22x, a, F11.3, 1x, a)') 'PDAF', 'LNETF analysis (3):', pdaf_time_tot(3), 's'
          WRITE (*, '(a, 18x, a, F11.3, 1x, a)') 'PDAF', 'prepare observations (6):', pdaf_time_tot(6), 's'
          WRITE (*, '(a, 22x, a, F11.3, 1x, a)') 'PDAF', 'init_dim_obs_pdaf (43):', pdaf_time_tot(43), 's'
          WRITE (*, '(a, 28x, a, F11.3, 1x, a)') 'PDAF', 'obs_op_pdaf (44):', pdaf_time_tot(44), 's'
          WRITE (*, '(a, 26x, a, F11.3, 1x, a)') 'PDAF', 'init_obs_pdaf (50):', pdaf_time_tot(50), 's'
          WRITE (*, '(a, 17x, a, F11.3, 1x, a)') 'PDAF', 'compute ensemble mean (9):', pdaf_time_tot(9), 's'
          WRITE (*, '(a, 19x, a, F11.3, 1x, a)') 'PDAF', 'global preparations (7):', pdaf_time_tot(7), 's'
!        IF (dim_lag>0) &
!             WRITE (*, '(a, 14x, a, F11.3, 1x, a)') 'PDAF', 'compute HX for smoother (27):', pdaf_time_tot(27), 's'
!         IF (type_forget==0 .OR. type_forget==1) &
!              WRITE (*, '(a, 21x, a, F11.3, 1x, a)') 'PDAF', 'inflate ensemble (14):', pdaf_time_tot(14), 's'
!        WRITE (*, '(a, 14x, a, F11.3, 1x, a)')  'PDAF', 'compute HX for filter (12):', pdaf_time_tot(12), 's'
          WRITE (*, '(a, 15x, a, F11.3, 1x, a)') 'PDAF', 'generate random matrix (33):', pdaf_time_tot(33), 's'
          WRITE (*, '(a, 19x, a, F11.3, 1x, a)') 'PDAF', 'local analysis loop (8):', pdaf_time_tot(8), 's'
          WRITE (*, '(a, 24x, a, F11.3, 1x, a)') 'PDAF', 'global to local (10):', pdaf_time_tot(15), 's'
          WRITE (*, '(a, 18x, a, F11.3, 1x, a)') 'PDAF', 'localize observations (11):', pdaf_time_tot(11), 's'
          WRITE (*, '(a, 22x, a, F11.3, 1x, a)') 'PDAF', 'init_dim_obs_l_pdaf (38):', pdaf_time_tot(38), 's'
          WRITE (*, '(a, 29x, a, F11.3, 1x, a)') 'PDAF', 'g2l_obs_pdaf (46):', pdaf_time_tot(46), 's'
          WRITE (*, '(a, 31x, a, F11.3, 1x, a)') 'PDAF', 'init_obs_l (47):', pdaf_time_tot(47), 's'
          WRITE (*, '(a, 25x, a, F11.3, 1x, a)') 'PDAF', 'local analysis (12):', pdaf_time_tot(12), 's'
          WRITE (*, '(a, 25x, a, F11.3, 1x, a)') 'PDAF', 'compute matrix A (16):', pdaf_time_tot(16), 's'
          WRITE (*, '(a, 19x, a, F11.3, 1x, a)') 'PDAF', 'compute particle weights (20):', pdaf_time_tot(20), 's'
          WRITE (*, '(a, 28x, a, F11.3, 1x, a)') 'PDAF', 'likelihood_l_pdaf (48):', pdaf_time_tot(48), 's'
          WRITE (*, '(a, 33x, a, F11.3, 1x, a)') 'PDAF', 'complete A (21):', pdaf_time_tot(21), 's'
          WRITE (*, '(a, 24x, a, F11.3, 1x, a)') 'PDAF', 'compute transform (17):', pdaf_time_tot(17), 's'
          WRITE (*, '(a, 35x, a, F11.3, 1x, a)') 'PDAF', 'SVD of A (22):', pdaf_time_tot(22), 's'
!        WRITE (*, '(a, 25x, a, F11.3, 1x, a)') 'PDAF', 'product of T and A (32):', pdaf_time_tot(32), 's'
!        WRITE (*, '(a, 24x, a, F11.3, 1x, a)') 'PDAF', 'multiply with svals (33):', pdaf_time_tot(33), 's'
!        IF (type_forget==2 .OR. type_forget==3) &
!             WRITE (*, '(a, 22x, a, F11.3, 1x, a)') 'PDAF', 'inflate post ensemble (34):', pdaf_time_tot(34), 's'
!        WRITE (*, '(a, 24x, a, F11.3, 1x, a)') 'PDAF', 'apply random matrix (35):', pdaf_time_tot(35), 's'
          ...
          IF (dim_lag >0) &
               WRITE (*, '(a, 22x, a, F11.3, 1x, a)') 'PDAF', 'perform smoothing (15):', pdaf_time_tot(15), 's'
          WRITE (*, '(a, 16x, a, F11.3, 1x, a)') 'PDAF', 'add noise to ensemble (19):', pdaf_time_tot(19), 's'
