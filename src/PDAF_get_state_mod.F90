...
MODULE PDAF_get_state_mod
  ...
  ABSTRACT INTERFACE 
     SUBROUTINE prepoststep(step, dim_p, dim_ens, dim_ens_p, dim_obs_p, &
       ...
       REAL, INTENT(inout) :: Uinv(dim_ens_p-1, dim_ens_p-1)  !< Inverse of matrix U
...
SUBROUTINE PDAFget_state(steps, time, doexit, U_next_observation, U_distribute_state, &
...
!  (PDAF-internal names, real names are defined in the call to PDAF)
  PROCEDURE(next_observation) :: U_next_observation
  PROCEDURE(distribute_state) :: U_distribute_state
  PROCEDURE(prepoststep) :: U_prepoststep
...
END SUBROUTINE PDAFget_state

END MODULE PDAF_get_state_mod
