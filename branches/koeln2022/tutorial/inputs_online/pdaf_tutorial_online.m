...
dim_x = 12;         % Grid dimension in x-direction
dim_y = 12;         % Grid dimension in y-direction
dim_ens = 4;        % Maximum ensemble size
dim_step = 8;      % Number of time steps
stddev_obs = 0.5;   % Observation error standard deviation
dxobs = 4;          % x-Grid spacing for observations type A
dyobs = 4;          % y-Grid spacing for observations type A
dxobsB = 4;        % x-Grid spacing for observations type B
dyobsB = 4;         % y-Grid spacing for observations type B
...
for j=1:dim_x
    for i=1:dim_y
        field(i,j,1) = sin(2*pi*(i/12+j/12));
    end
end
...
for k=1:dim_ens
    if k==1
        for j=1:dim_x
            for i=1:dim_y
                ens(i,j,k) = sin(2*pi*(i/12+j/12)+2*0.5*pi*(k+5)/dim_ens);
            end
        end
    elseif k ==2
        for j=1:dim_x
            for i=1:dim_y
                ens(i,j,k) = sin(2*pi*(i/12+j/12)+1.5*2*0.5*pi*(k+5)/dim_ens);
            end
        end
    elseif k ==3
        for j=1:dim_x
    ...
    elseif k ==4
        for j=1:dim_x
            for i=1:dim_y
                ens(i,j,k) = sin(2*pi*(i/12-j/12)+1.7*2*0.5*pi*(k+5)/dim_ens);
            end
        end
    elseif k ==5
        for j=1:dim_x
            for i=1:dim_y
                ens(i,j,k) = sin(2*pi*(i/12+0/12)+2*0.5*pi*(k+5)/dim_ens);
            end
        end
    elseif k ==6
        for j=1:dim_x
            for i=1:dim_y
                ens(i,j,k) = sin(2*pi*(i/12+0/12)+1.7*2*0.5*pi*(k+5)/dim_ens);
            end
        end
    end         
    figure
