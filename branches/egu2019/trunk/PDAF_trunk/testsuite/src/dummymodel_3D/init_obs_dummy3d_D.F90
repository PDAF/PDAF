!$Id: init_obs_dummy3d_D.F90 783 2009-12-07 10:28:43Z lnerger $
!BOP
!
...
! !USES:
  ...
  USE mod_assimilation, &
       ONLY: n_obs, local_dims_obs, rms_obs
  USE mod_model, &
       ONLY: dt
...
!      END DO
     ...
     CALL dlarnv(3, iseed, n_obs, obs_errors(1 : n_obs))
...
! ****************************
  ...
  mype0b: IF (mype_filter == 0) THEN
     ...
     DO domain = 1, npes_filter
        ...
        ELSE whichdomain
           ...
           ALLOCATE(observation_temp_p(local_dims_obs(domain)))

           ! perform reordering of observation vector
           DO i = 1, local_dims_obs(domain)
              observation_temp_p(i) = observation(i + offset)
           END DO

           ! Send sub-arrays
           CALL MPI_send(observation_temp_p, local_dims_obs(domain), &
        ...
        ! Increment offset
        offset = offset + local_dims_obs(domain)
  ...
  ELSE mype0b
     ...
     CALL MPI_recv(observation_p, local_dims_obs(mype_filter + 1), &
