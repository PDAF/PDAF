...
!BOP
!
! !ROUTINE: PDAF_ewpf_alloc --- PDAF-internal initialization of EWPF
!
! !INTERFACE:
SUBROUTINE PDAF_ewpf_alloc(subtype, outflag)

! !DESCRIPTION:
! Perform allocation of arrays for EWPF.
!
...
! !USES:
  USE PDAF_memcounting, &
       ONLY: PDAF_memcount
  USE PDAF_mod_ewpf, &
       ONLY: screen, dim_ens, dim_p, dim_lag, &
       state, eofU, eofV, sens, weights
...
! ******************************
  ...
  on_filterpe: IF (filterpe) THEN
    ...
    ALLOCATE(state(dim_p), stat = allocstat)
     ...
     ! PF weights are stored on filter pes
     ALLOCATE(weights(dim_ens), stat = allocstat)
     IF (allocstat /= 0) THEN
        WRITE (*,*) 'PDAF-ERROR(20): error in allocation of weights'
        outflag = 20
     END IF
     ! count allocated memory
     CALL PDAF_memcount(2, 'r', dim_ens) 
 
     ! initialize weights
     weights = -log(1./dim_ens)
 ...
 ELSE on_filterpe
     ...
     ! Model-PEs that are not Filter-PEs
     !  only need a single weight calculated in the proposal step
     ALLOCATE(weights(1), stat = allocstat)
     IF (allocstat /= 0) THEN
         WRITE (*,*) 'PDAF-ERROR(20): error in allocation of weights'
         outflag = 20
     END IF
        ! count allocated memory
     CALL PDAF_memcount(2, 'r',1)

     weights = -log(1./dim_ens)     ! initialize weights

  END IF on_filterpe

END SUBROUTINE PDAF_ewpf_alloc
