...
! For EWPF:
  EXTERNAL :: prodQx, &             ! Compute product of model error covariance Q with a vector
       get_modelerror, &            ! Initialize a single realization of random model error
       adj_obs_op_pdaf, &           ! Apply ajoint observation operator
       solve_invHQHTpR              ! Compute and invert HQH^T + R
...
! *********************************
  ...
  ELSEIF (filtertype == 12) THEN
     CALL PDAF_assimilate_ewpf(collect_state_pdaf, distribute_state_pdaf,&
          init_dim_obs_f_pdaf, obs_op_f_pdaf, init_obs_f_pdaf,&
          prepoststep_ens_pdaf, prodRinvA_pdaf, next_observation_pdaf,&
          prodQx, get_modelerror, adj_obs_op_pdaf, &
          solve_invHQHTpR, status_pdaf)
  END IF
