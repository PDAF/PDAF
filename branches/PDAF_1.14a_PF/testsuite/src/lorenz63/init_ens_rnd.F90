!$Id: init_ens_rnd.F90 61 2019-02-01 08:49:36Z lnerger $
!BOP
!
...
!
! The routine is called by init_ens. It 
...
! *** Determine index interval to scale random values
  ...
  IF (init_maxtime > nsteps_file) THEN
     init_maxtime = nsteps_file
     write (*,*) 'Reset init_maxtime to steps in file: ', init_maxtime
  ENDIF

  ! Maximum number of indices
  max_indx = (init_maxtime - step_null)/init_dt
  ...
  WRITE (*, '(9x,a,i5,a,i7)') &
       '---    with time step interval of ',init_dt,' time steps up to time step ', init_maxtime
  WRITE (*, '(9x,a,i3)') &
...
! *** Initialize array of random indices
  ...
  DO i = 1, dim_ens
     rnd_indx(i) = init_dt*INT(rnd_array(i) * max_indx) + step_null
     IF (rnd_indx(i) > init_dt*max_indx+step_null) rnd_indx(i) = max_indx
